...
col1, col2, col3, col4, col5 = st.columns(5)
with col1:
    final_third_dribbles_per90 = (
                player_data.get('Dribles no último terço do campo com sucesso', 0) * 90 / minutes) if minutes > 0 else 0
...
def create_simple_radar_chart_fixed(players_data: List[Dict], selected_metrics: List[str],
    ...
    # Use a specific container to prevent navigation issues
    radar_container_key = f"{unique_key}_radar_container"
    ...
    for i, player_data in enumerate(players_data):
        ...
        fig.add_trace(go.Scatterpolar(
            ...
            fillcolor=colors[i % len(colors)].replace('#', 'rgba(').replace('FF6B6B', '255, 107, 107').replace('4ECDC4',
                                                                                                               '78, 205, 196').replace(
                '45B7D1', '69, 183, 209').replace('96CEB4', '150, 206, 180').replace('FECA57',
                                                                                     '254, 202, 87') + ', 0.3)',
            opacity=0.7
        ))
